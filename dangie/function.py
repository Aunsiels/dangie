...
class Function(object):
    ...
    def _insert_atom(self, atom):
        self.atoms.add_edge(atom[INPUT_VARIABLE], atom[OUTPUT_VARIABLE],
    ...
    def get_linear_paths(self):
        ...
        for target_node in self.atoms.nodes:
            if (target_node != self.start_node and target_node not in
    ...
    def get_all_paths_to(self, target_node):
        linear_paths = []
        for unlabeled_path in nx.all_simple_paths(self.atoms,
                                                  self.start_node,
                                                  target_node):
    ...
    def get_all_relation_paths_from_unlabeled_path(self, unlabeled_path):
        ...
        for next_node in unlabeled_path[1:]:
            labeled_paths = \
                self.explore_all_relations_between_two_nodes(
                    next_node, previous_node, labeled_paths)
            previous_node = next_node
        return labeled_paths

    def explore_all_relations_between_two_nodes(self, next_node, previous_node,
                                                previous_plans):
        next_plans = []
